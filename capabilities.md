...
| `signatureModal` | The canvas `<dialog>` the decision bar opens when Approve has no signature on file yet (2026-09-19) -- draw, optionally save, submit as part of the same `hx-post .../decide` | `reviewdocument.templ` (moved from `detail.templ` in Fase 6b with its markup unchanged); `reviewDecisionBar`, shown only when `HasSignature` is false |
...
| `mch_approval_step` | An approval step in a document workflow | document, sequence, step name, assignee, decision, signature page/x/y/width, signature image | `/decide` Action, sequencing enforced, `prm_decide_own_step`; `fld_signature_image` is a one-time signature captured at Approve time when its assignee chose not to save it as their own reusable `mch_signature` (2026-09-19); `fld_step_name` is what a step is *for* ("Finance Review") independent of who holds it — declared in Fase 6b for board 10's step titles, written by nothing until board 08's wizard in 6c, with `composition.stepLabel` falling back to the assignee's name meanwhile |
