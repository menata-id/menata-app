...
| `SettingsHub` / `SettingsHubMember` | A navigation item marks itself as an Application's Settings landing page, or a member reached from it (e.g. Permissions) rather than from the main menu — both excluded from `appshell.templ`'s tab strip/bottom bar either way | Built (2026-09-25) | `metadata/applications/document-approval.yaml`'s `nav_app_settings` / `nav_app_settings_permissions`, `domain.NavigationItem.SettingsHub`/`SettingsHubMember`, `appshell.templ`'s `settingsHubItem` (a search over `Application.AllNavigation`, not a fixed id, so an Application declaring no hub — Project Management, today — is unaffected rather than causing a panic). Deliberately two booleans, not a string naming which section an item belongs to (an earlier draft of this pair, `SettingsSection`): the hub has exactly one real section ("Access") today, and asking an author to also classify each item by hand would restate a fact `composition.ApplicationSettingsHub` already derives from `len(app.Roles) > 0`. The route still has to be declared either way — this runtime resolves "which Application" a request belongs to by matching a declared route, the same reason `nav_my_documents`/`nav_assigned_to_me` are each declared despite sharing one physical handler — but whether the Access section's rows actually render is computed from `Roles` and the viewer's own Workspace role, never from a nav item's mere existence, so an Application that later empties its `roles:` cannot leave a dead-looking row behind |
...
| Approval Inbox (one-screen worklist, SLA filter chips, plus "My Documents" and "Assigned to me") | Built, hardcoded to one Machine pair | `GET /approval-inbox` — `rendering.ApprovalInboxPage`, `composition/approval.go`; submitter resolved from `mch_activity`, not a Field; every card carries a `DOC-0091`-style `Reference` (derived from `sort_order`) and `mode · N/M approved`; a worklist/My Documents card is a plain navigation to the record's own detail page (2026-09-19: dropped the inline hx-get/hx-target swap this used to layer on top -- a click opens its own page, not a pane below the list). **Assigned to me** (`?tab=assigned`, 2026-09-24, board 07b): every Approval Step that has ever named this viewer -- directly, or through a Group they belong to (CAP-F24's two-armed test, `composition.stepBelongsTo`) -- whatever its own decision, newest submission first; a still-pending step reads "waiting for your decision" or "not yet your turn" by the same `behavior.CanAct` the decision bar itself asks. Deliberately bespoke Go rather than a declared `where:` filter: the field below explains why |
...
| `reviewDecisionBar` / `reviewProgress` / `reviewDocumentCard` / `reviewStepMarker` | Board 10's own panels: SLA + Approve/Reject, the Approval Progress list, and the file card with View PDF / Download. Page-internal and marked so on purpose (Q1: one caller) -- `reviewProgress` is deliberately *not* `approvalStepper`, which still serves the Document detail page in `pageStyles`; they share the composed `[]StepApprover` and not the markup, because Preflight ties chrome to content | `reviewdocument.templ`; `ReviewDocumentPage` |
| `reviewStatusPill` | A Document's own `fld_status` value as a coloured pill -- the three known values get their own colour, anything else falls through to neutral grey rather than being hidden. **Promoted out of the page-internal row above on 2026-09-24**, the second caller this table's own criterion asks for: Assigned to me's own table (`assignedTable`) draws a Document's status the identical way board 10 does, rather than inventing a second status pill for a screen that shows the same fact | `reviewdocument.templ`; `ReviewDocumentPage`'s own header, and `approvalinbox.templ`'s `assignedTable` |
| `roleMatrixApp` | One Application's own card in the Authorization Matrix: a header naming its role vocabulary and linking to Workspace Members, then a two-column "What you can do" / "Who can do it" list per Machine -- the same responsive shape `RoleMatrixPage`'s own Workspace section already uses for `RoleMatrixNote`, not a grid (rewritten from a role-per-column `<table>` on 2026-09-25 to match the Flow 2 mockup's `RoleMatrix.dc.html`/`M06-RoleMatrix.dc.html`, neither of which draw a grid at either breakpoint). **A real second caller, not a page-internal shape**: `composition.RoleMatrixForApplication` (an exported wrapper around the unexported `applicationBlock` both callers share) feeds the identical component to the Application Settings hub's own Permissions view, so `/authorization-matrix`'s multi-Application overview and `/document-approval/settings/permissions`'s single-Application detail read from one content pipeline rather than two | `rolematrix.templ`; `RoleMatrixPage` (every Application at once) and `appsettings.templ`'s `ApplicationSettingsPage` (one Application, reused as its content pane on desktop and its detail view on mobile) |
