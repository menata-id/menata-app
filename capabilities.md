...
| | Stylesheet | Pages |
|---|---|---|
| **Tailwind** | `static/css/app.css`, built from `static/css/input.css` by `make css` | The seven pre-auth screens (`authShell`); the Workspace-level screens (`appShell`): Workspace Home, Workspace Members, Edit member, Groups, Group detail; and the Application screens ported so far — Approval Inbox (Fase 6a), Review Document (6b), the submission wizard (6c-2) and Signature positions (6c-3). **Case 3 is fully ported**; `pageStyles` now serves only the generic record pages and the Project Management screens |
