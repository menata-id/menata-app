...
`metadata/app.yaml` has no `workspace.navigation:` block any more, by owner instruction. A
...
One consequence left standing deliberately: `membersHiddenFor` still returns the three
Workspace-administration ids, but nothing filters on them any more — `hiddenNavIDs` now only
reaches `launcherAppItems`, which sees Application items only. The plumbing is still live for
Application items; the three Workspace ids in that list are inert. Removing them (and possibly the
whole parameter) is a separate change, not folded into this one.
