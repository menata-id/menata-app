...
| `appShell` / `appLauncher` / `launcherAppCard` / `launcherAppGroup` / `launcherLink` / `headMeta` | The Workspace-level page frame (`ui-sample/case-03-flow1` boards 03–05): a 9-dot launcher, a `Workspace / Page` breadcrumb and the viewer's avatar. Redesigned 2026-09-20 (owner request) from a flat list of every declared item into `ui-sample/nav-chrome.js`'s own card shape: the panel opens on a "WORKSPACE" header row linking to Workspace Home, then the Workspace's own remaining destinations (Workspace Members, Groups — All Machines is deliberately dropped from this one entry point, `workspaceLauncherItems`'s own doc comment), then one entry per `Workspace.Applications`, and closes on an "All Workspaces →" link to `/switch-workspace` -- shown for any real identity (`internal/web.viewerWorkspaceContext`'s `switchHref`), unconditionally rather than only when there is a second Workspace to switch to, since that screen is where an "add workspace" entry point is meant to land next (owner request, 2026-09-20); omitted only for the shared admin credential's placeholder identity, which has no membership row to key a Workspace list on. An Application's own entry is a single `launcherAppCard` (icon, name, description, linking to its `HomeRoute`) when it declares `show_nav: true` — it keeps its own menu chrome once you're inside it (`pageShell`'s topbar), so the launcher only has to get you there — or `launcherAppGroup`, the pre-redesign flat item list, when it declares `show_nav: false` (Document Approval: `appShell` screens with no topbar of their own), because the launcher is deliberately the only place those screens stay reachable. Both branches read `Application.AllNavigation` — the list *before* `hidden_nav_groups` filtering — for the same reason: hiding a group suppresses its menu chrome, never its launcher entry. `headMeta` is the `<head>` shared with `authShell`. `appShell`'s `hiddenNavIDs` parameter is per-*viewer* filtering, a different question from the metadata-level `hidden_nav_groups` — see the navigation note below | `appshell.templ`; every `appShell`-based screen |
...
| | Stylesheet | Pages |
|---|---|---|
| **Tailwind** | `static/css/app.css`, built from `static/css/input.css` by `make css` | The seven pre-auth screens (`authShell`); the Workspace-level screens (`appShell`): Workspace Home, Workspace Members, Edit member, Groups, Group detail; and the Application screens ported so far — Approval Inbox (Fase 6a), Review Document (6b), the submission wizard (6c-2) and Signature positions (6c-3). **Three Case 3 screens are still `pageStyles`, and one of them is a declared navigation item**: `/dashboard` (`nav_dashboard`, priority 1 — the first entry in Document Approval's own menu; its board is marked UNUSED by the owner, so there is no mockup to port it against), plus the Document and Approval Step detail pages, which are `detail.templ` — the *generic* record view every Machine shares, so porting it reaches far beyond Case 3. The visible cost today is a shell change mid-flow: the Signature positions screen's back link lands on an old-style page |
...
deliberately does *not* reach the launcher, which is why `appLauncher` reads `AllNavigation`. That
same `show_nav` field now also decides *how* the launcher itself presents an Application (a
compact `launcherAppCard` when true, the flat `launcherAppGroup` item list when false) — a second,
deliberate reuse of one metadata field to answer a related but different question, not a
contradiction of "the launcher never reads it": it still never reads it to decide *whether* an
Application appears, only *how*.
...
|---|---|---|---|
| `mch_user` | Real identity | name, email, weekly capacity | `prm_edit_user_is_admin`/`prm_delete_user_is_admin` — changing the member directory is Workspace administration (`workspace_role: admin`). Credentials are not here; they live in the platform `credentials` table |
...
| `mch_signature` | Approver's own reusable signature image | owner, image | `prm_create_own_signature`/`prm_edit_own_signature`/`prm_delete_own_signature`, all `actor_field: fld_owner` — deliberately no role, since owning your own signature is not a privilege an Application grants. Input to the PDF-compositing Action; created by `decideButtons`' own canvas modal when its "save my signature" checkbox is checked (2026-09-19) -- previously only reachable through the generic Machine CRUD form |
...
| Mobile bottom bar not built (004, 006) | The cross-Application launcher (`appShell`'s 9-dot menu) is built and card-shaped since 2026-09-20 — see the "Shared rendering components" row above. `ui-sample/README.md`'s other mobile-specific piece of the two-level nav, the bottom tab bar `nav-chrome.js`'s own `mount()` renders for a narrow viewport, has no counterpart here yet |
...
| Navigation is not per-user/role-filtered (006 §Navigation, 007 §20) | `application.navigation` (`app.yaml`) is resolved once at process startup into package-level state (`internal/rendering.ConfigureNavigation`), identical for every request/viewer; `domain.NavigationItem` has no role/permission field. **This row used to say "no declared nav item needs this yet", and cited a fix in `workspaceHomeShell` — a function that no longer exists.** Fase 2 dissolved both halves: the launcher renders `application.navigation` itself, so `nav_workspace_members` *is* a declared item pointing at a `requireWorkspaceAdmin`-gated route. What exists is a viewer-level stand-in, not the declared form — `appShell`'s `hiddenNavIDs` with the handler naming the item it already gates (see "Navigation: two filters" above). `requires_role:` on a navigation item stays unbuilt, waiting on a *second* real case (ROADMAP.md, Planned) |
