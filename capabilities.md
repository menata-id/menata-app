...
| `GET /approval-role-matrix` | Board 06, the Approval Role Matrix: which role may perform which declared transition, for one Application (`?app=` selects it). Read-only, metadata-only — no store call at all — and behind `requireWorkspaceAdmin` like the two rows above |
...
`/workspace-members*`, `/workspace-groups*` and `/approval-role-matrix` additionally require
