...
| SLA badges (`sla_field`) | **A Page's own primary action** — the Approval Inbox's "+ New Document" button. `navigation:` declares menu *destinations*; it cannot say "this screen has a call to action pointing at that screen". `nav_new_approval` was deleted on 2026-09-21 (owner instruction) precisely because declaring a submit form as a menu item was the wrong shape for it, which left the route and the label as literals in `approvalinbox.templ` and `documentsubmit.templ`, and an entry in `internal/conformance`'s `applicationSubScreens`. Forward-checkable pointer: 007 §12.3's `ActionBar` component, still unbuilt |
