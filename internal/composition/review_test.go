...
func TestBuildReview_DecisionBarIsOfferedToItsOwnAssigneeOnly(t *testing.T) {
	...
	got := buildReview(steps[0], document, steps, nil, users, stepMachineForTest(), docMachineForTest(), domain.Actor{ID: "usr_budi"}, 6, true, at(10))
	if !got.CanDecide {
	...
	if got := buildReview(steps[1], document, steps, nil, users, stepMachineForTest(), docMachineForTest(), domain.Actor{ID: "usr_ana"}, 6, true, at(10)); got.CanDecide {
	...
	if got := buildReview(steps[0], document, steps, nil, users, stepMachineForTest(), docMachineForTest(), domain.Actor{ID: "usr_ana"}, 6, true, at(10)); got.CanDecide {
	...
	if got := buildReview(decided, document, []*data.Record{decided}, nil, users, stepMachineForTest(), docMachineForTest(), domain.Actor{ID: "usr_budi"}, 6, true, at(10)); got.CanDecide {
...
func TestBuildReview_StepLabelFallsBackToAssignee(t *testing.T) {
	...
	got := buildReview(s, reviewDoc(), []*data.Record{s}, nil, users, stepMachineForTest(), docMachineForTest(), domain.Actor{ID: "usr_ana"}, 0, true, at(10))
...
func TestBuildReview_MarksOnlyTheViewersOwnStep(t *testing.T) {
	...
	got := buildReview(steps[1], reviewDoc(), steps, nil, users, stepMachineForTest(), docMachineForTest(), domain.Actor{ID: "usr_ana"}, 0, true, at(10))
	if len(got.Steps) != 3 {
...
func TestBuildReview_PlacementNeedsAPage(t *testing.T) {
	s := step("stp_1", "doc_1", "usr_ana", action.DecisionPending, 1)
	got := buildReview(s, reviewDoc(), []*data.Record{s}, nil, users, stepMachineForTest(), docMachineForTest(), domain.Actor{ID: "usr_ana"}, 6, true, at(10))
...
func TestBuildReview_SLAIsDayScale(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got := buildReview(s, document, []*data.Record{s}, nil, users, stepMachineForTest(), docMachineForTest(), domain.Actor{ID: "usr_ana"}, 0, true, at(10))
...
func TestBuildReview_FileCardHandlesAMissingFile(t *testing.T) {
	...
	if got := buildReview(s, bare, []*data.Record{s}, nil, users, stepMachineForTest(), docMachineForTest(), domain.Actor{ID: "usr_ana"}, 0, true, at(10)); got.FileHref != "" {
