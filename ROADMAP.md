...
- **The three declared roles now mean three different things** (owner decision, 2026-09-21):
  ...
  **Live effect, stated because it is real**: of nine pending Approval Steps in the dev Workspace,
  ...
  need the role granted, directly or through a Group.
...
- **Porting the Case 03 Flow 1 design** (owner mockup, 2026-09-20, unpacked into
  ...
  **Re-reading this table is part of closing a phase, not a tidy-up.** That is written here
  ...
  | The wizard's approver picker offers every Workspace member, including people who can never approve | **no phase yet** — **and the reviewer decision of 2026-09-21 is what made it bite** | `fld_assignee` resolves through the generic relation-option loader, so the submit wizard lists every `mch_user` record. With `decide` now gated on the `approver` role, a submitter can build a chain naming someone who holds `reviewer` or nothing — producing a step that is **assigned to a real person and decidable by nobody**, with no error at submit time and no hint on the screen. Upstream solved exactly this on its own `user` Field (CAP-F05 ✅): the picker is narrowed at query time to people holding a role in the field's own Machine's Application, *"a query-time filter, no new metadata concept"*. That is the shape to implement; what it needs here is for `composition.Loader.RelationOptions` to know the Application and the role a Permission requires, neither of which it is given today. **Until it lands, a dead step is created silently** — the closest thing to a guard is this row |
